...
function requestEditArtice(articleData, serverUrl, articleId, page) {
  //2. Set up the request
  ...
  //3. Execute the request
  fetch(`${serverUrl}/${articleId}`, postReqSettings) //now we need the second parameter, an object wih settings of the request.
    .then(response => {
      //fetch promise fullfilled (operation completed successfully)
      if (response.ok) {
        //successful execution includes an error response from the server. So we have to check the return status of the response here.
...
function requestInsertArticle(articleData, serverUrl) {
  const postReqSettings =
    //an object wih settings of the request
    {
      method: "POST",
  ...
  //3. Execute the request
  fetch(`${serverUrl}`, postReqSettings) //now we need the second parameter, an object wih settings of the request.
    .then(response => {
      //fetch promise fullfilled (operation completed successfully)
      if (response.ok) {
        //successful execution includes an error response from the server. So we have to check the return status of the response here.
    ...
    .then(responseJSON => {
      ...
      window.alert("Inserted article successfully saved on server");
      window.location.hash = `#article/` + responseJSON.id + `/0`;
    })
    .catch(error => {
      ////here we process all the failed promises
      window.alert(`Inserted article failed to save on server. ${error}`);
    });
 ...
 * @param offset - currentemplate-article-formt offset of the article list display to which the user should return
 * @param totalCount - total number of articles on the server
 * @param serverUrl - basic part of the server url, without the service specification, i.e.  https://wt.kpi.fei.tuke.sk/api.
 */

//chceme odoslat formular
function processArtEditFrmData(event, articleId, page, serverUrl, inserting) {
  ...
  if (inserting) {
    requestInsertArticle(articleData, serverUrl);
  } else {
    requestEditArtice(articleData, serverUrl, articleId, page);
  }
}

function processCommentsFormData(event, articleId, serverUrl) {
  event.preventDefault();
  ...
  requestInsertComment(commentData, serverUrl, articleId);
}

function requestInsertComment(commentData, serverUrl, articleId) {
  const postReqSettings =
    //an object wih settings of the request
    {
      method: "POST",
  ...
  console.log(`${serverUrl}/api/comment/${articleId}`);

  //3. Execute the request
  fetch(`${serverUrl}/comment`, postReqSettings) //now we need the second parameter, an object wih settings of the request.
    .then(response => {
      //fetch promise fullfilled (operation completed successfully)
      if (response.ok) {
        //successful execution includes an error response from the server. So we have to check the return status of the response here.
    ...
    .then(responseJSON => {
      //here we process the returned response data in JSON ...
      console.log(responseJSON);
      window.alert("Inserted comment successfully saved on server");
    ...
    .catch(error => {
      ////here we process all the failed promises
      window.alert(`Inserted comment failed to save on server. ${error}`);
    });
  //.finally(() => window.location.hash = `#articles/0`);
}
